...
import static cy.ac.ouc.cognition.nestor.lib.trace.Trace.TraceLevel;
...
public class StanfordCoreNLProcessor extends NLProcessor implements INLProcessor {
    ...
    private static StanfordCoreNLProcessor	StanfordCoreNLPInstance;

    private	Properties			CoreNLPProperties;
	private	StanfordCoreNLP		CoreNLPPipeline;
	private CoreDocument		CoreNLPDocument;
	
	File tempFile;

	private StanfordCoreNLProcessor() {
		super();
	}

	

    public static synchronized StanfordCoreNLProcessor getInstance(boolean forceNew) {

    	if (forceNew || StanfordCoreNLPInstance == null) {

        	outln("Create new StanfordCoreNLProcessor instance!");

        	StanfordCoreNLPInstance = new StanfordCoreNLProcessor();
	...
	public void load() {
		...
		CoreNLPProperties.setProperty("annotators", getSysParameters().getCoreNLP_Annotators());

		// Set a property for coref algorithm annotator 
		CoreNLPProperties.setProperty("coref.algorithm", getSysParameters().getCoreNLP_Algorithm());
		
		// Set a property for how to split document to sentences 
		CoreNLPProperties.setProperty("ssplit.eolonly", getSysParameters().getCoreNLP_Split());
		
		CoreNLPProperties.setProperty("ner.useSUTime", "0");

		// Check if regexner annotator is set
		if (isRegexnerAnnotatorSet()) {
			
			final String mappingFileName = "NESTORregexner.tab";
			
			try {

				File mappingFile = new File(mappingFileName);
				
				if (Files.isReadable(mappingFile.toPath()))
					// Set custom regexner mapping file
					CoreNLPProperties.setProperty("regexner.mapping", mappingFileName);
				else
					errln("Could not set Regexner mapping: Mapping file not readable");		

			}
			
			catch (Exception e) {
				errln("Could not set Regexner mapping: " + e.getMessage());		
			}
		...
		outln(TraceLevel.IMPORTANT, "Stanford Core NLP Properties Set: " + CoreNLPProperties.toString());
...
//			        }
		...
		for (CoreSentence coreNLPSentence : CoreNLPDocument.sentences()) {
			...
			for (CoreLabel coreNLPToken : coreNLPSentence.tokens()) {
				...
				String originalText = coreNLPToken.originalText();
				...
				if (originalText != null && !originalText.isEmpty() &&
					Character.isUpperCase(originalText.charAt(0)))
					coreNLPToken.setNER("forced");
				*/
				...
				if (CoreNLPProperties.getProperty("annotators").contains("coref") &&
					tag.toLowerCase().matches(getTranslationParameters().getNLP_CreateCorefPOS().toLowerCase())) {
	...
	public String getNLPData(NLDocument nlDocument) {
	...
	boolean isRegexnerAnnotatorSet()
	{
		return getSysParameters().getCoreNLP_Annotators().contains("regexner");
	}
